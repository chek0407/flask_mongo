...
# JWT configuration
app.config['JWT_SECRET_KEY'] = 'your-secret-key' # Use a strong, unique key in production
jwt = JWTManager(app)
...
def add_user():
    ...
    try:
        ...
        return jsonify({'message': 'User added successfully', 'user': fix_decimals(item)}), 201
    except PyMongoError as e:
        logging.error(f"MongoDB Error: {e}", exc_info=True)
...
def update_user(user_id):
    ...
    try:
        ...
        if update_result.modified_count == 0:
            return jsonify({'message': 'No attributes updated or user data is the same'}), 200

        # Retrieve the updated document to return it
        updated_user = users_collection.find_one({'_id': user_id})
        updated_user['UserId'] = updated_user.pop('_id') # Restore UserId for consistency
        return jsonify({'message': 'User updated successfully', 'updatedAttributes': fix_decimals(updated_user)}), 200
    except PyMongoError as e:
        logging.error(f"MongoDB Error: {e}", exc_info=True)
        return jsonify({'error': str(e)}), 500
    except Exception as e:
...
player_model = epl_ns.model('Player', {
    'PlayerID':   fields.String(required=True, description='Player unique identifier within team'), # Added PlayerID as per user JSON
...
class TeamList(Resource):
    ...
    def get(self):
        ...
        try:
            ...
            teams = list(epl_collection.find({})) # Now fetching all documents as they are teams with players
...
class TeamWithPlayers(Resource):
    ...
    def get(self, team_id):
        ...
        current_user = get_jwt_identity()
        logging.info(f"User '{current_user}' attempting to get details for team ID '{team_id}'")
        try:
            team = epl_collection.find_one({"TeamID": team_id}) # Find the team document
            if not team:
            ...
            if sort_by:
                ...
                try:
                    if sort_by in ['Age', 'Number']:
                        filtered.sort(key=lambda x: x.get(sort_by, 0) if x.get(sort_by) is not None else -1, reverse=reverse)
                    else:
                        filtered.sort(key=lambda x: x.get(sort_by, ""), reverse=reverse)
                except Exception as e:
                    return {'error': f'Invalid sort_by field or data type for sorting: {sort_by} - {e}'}, 400
...
class AddTeam(Resource):
    ...
    def post(self):
        ...
        if epl_collection.find_one({"TeamID": team_data.get('TeamID')}):
            return {'message': f"Team with TeamID '{team_data.get('TeamID')}' already exists."}, 409
        ...
        try:
            ...
            team_data['id'] = str(result.inserted_id) # Add MongoDB's _id to the response
...
class TeamPlayers(Resource):
    ...
    def get(self, team_id):
        ...
        current_user = get_jwt_identity()
        logging.info(f"User '{current_user}' attempting to get players for team ID '{team_id}'")
        try:
    ...
    def put(self, team_id):
        ...
        try:
            ...
            if update_result.matched_count == 0:
                return {'error': f'Team {team_id} not found'}, 404
            if update_result.modified_count == 0:
                return {'message': 'No attributes updated or team data is the same'}, 200
            
            updated_team = epl_collection.find_one({"TeamID": team_id})
            if updated_team:
                updated_team['id'] = str(updated_team.pop('_id'))
                return {'message': 'Team updated successfully', 'updated': fix_decimals(updated_team)}, 200
            else:
...
class TeamResource(Resource):
    ...
    def delete(self, team_id):
        ...
        try:
            ...
            logging.info(f"Deleted {delete_result.deleted_count} team document for team {team_id}")
...
class PlayerAdd(Resource):
    ...
    def post(self):
        ...
        existing_player = epl_collection.find_one({
            "TeamID": team_id,
            "Players.PlayerID": player_data.get('PlayerID')
        })
        if existing_player:
            return {'message': f"Player with PlayerID '{player_data.get('PlayerID')}' already exists in team '{team_id}'"}, 409
        ...
        try:
            ...
            return {'message': 'Player added to team', 'player': fix_decimals(player_data)}, 201
        except PyMongoError as e:
            logging.error(f"MongoDB Error: {e}", exc_info=True)
            return {'error': str(e)}, 500
        except Exception as e:
...
@epl_ns.route('/players/<string:team_id>/<string:player_id>') # Changed to player_id instead of jersey_number
class PlayerResource(Resource):
    ...
    def put(self, team_id, player_id):
        ...
        current_user = get_jwt_identity()
        logging.info(f"User '{current_user}' attempting to update player '{player_id}' for team ID '{team_id}'")
        data = epl_ns.payload
        ...
        try:
            ...
            update_result = epl_collection.update_one(
                {"TeamID": team_id, "Players.PlayerID": player_id}, # Find the team document and a matching player
                {"$set": {f"Players.$.{k}": v for k, v in data.items()}} # Update fields in the matched player
            )

            if update_result.matched_count == 0:
                return {'error': 'Team or Player not found'}, 404
            if update_result.modified_count == 0:
                return {'message': 'No attributes updated or player data is the same'}, 200
            
            # Retrieve the updated document to return the updated player
            team_doc = epl_collection.find_one({"TeamID": team_id})
            updated_player = next((p for p in team_doc.get('Players', []) if p.get('PlayerID') == player_id), None)
            
            if updated_player:
                return {'message': 'Player updated successfully', 'updatedAttributes': fix_decimals(updated_player)}, 200
            else:
                return {'error': 'Failed to retrieve updated player (after successful update)'}, 500
    ...
    def delete(self, team_id, player_id):
        ...
        current_user = get_jwt_identity()
        logging.info(f"User '{current_user}' attempting to delete player '{player_id}' for team ID '{team_id}'")
        try:
            ...
            if update_result.modified_count == 0:
                return {'error': 'Team or Player not found, or player already deleted'}, 404
            
            return {'message': f'Player with PlayerID {player_id} deleted from team {team_id}'}, 200
        except PyMongoError as e:
            logging.error(f"MongoDB Error: {e}", exc_info=True)
            return {'error': str(e)}, 500
        except Exception as e:
...
class EPLSearch(Resource):
    ...
    @epl_ns.param('key', 'Attribute name to search (e.g., TeamName, Players.PlayerName, Players.Age)')
...
user_model = api.model('User', {
    'UserId': fields.String(required=True, description='The user ID (will be MongoDB _id)'),
    ...
    'CreatedAt': fields.String(required=False, description='The user creation timestamp')
})
...
class SwaggerUserSearch(Resource):
    ...
    @users_ns.response(200, 'Users found', api.model('UserSearchResult', {'message': fields.String, 'users': fields.List(fields.Nested(user_model))}))
    @users_ns.response(400, 'Bad request')
    @users_ns.response(404, 'No users found')
    def get(self):
